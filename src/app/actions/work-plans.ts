...
export const createWorkPlan = withAuth(
  {
    role: EDITOR_ROLES,
    guards: [
      { resource: 'project', field: 'projectId' },
      { resource: 'discipline', field: 'disciplineId' },
    ],
  },
  async (
    ctx,
...
export const updateWorkPlanStatus = withAuthOnly(
  { role: EDITOR_ROLES },
  async (
    ctx,
    planId: string,
...
export const addWorkPlanItem = withAuth(
  { role: EDITOR_ROLES },
  async (
    ctx,
    input: {
...
// Antes no exigía rol — política 2026-05-24: actions sin role check → EDITOR_ROLES
export const updateWorkPlanItemStatus = withAuthOnly(
  { role: EDITOR_ROLES },
  async (
    ctx,
    itemId: string,
...
export const removeWorkPlanItem = withAuthOnly(
  { role: EDITOR_ROLES },
...
export const deleteWorkPlan = withAuthOnly(
  { role: EDITOR_ROLES },
  async (ctx, planId: string): Promise<{ error?: string }> => {
    const { error } = await ctx.supabase
      .from('work_plans')
      .delete()
      .eq('id', planId)
...
export const getProjectPlanContext = withAuthOnly(
  {},
  async (
    ctx,
    projectId: string,
...
export const addItrsToWorkPlan = withAuthOnly(
  { role: EDITOR_ROLES },
  async (
    ctx,
    planId: string,
...
export const createWorkPlanAndAddItrs = withAuth(
  {
    role: EDITOR_ROLES,
    guards: [
      { resource: 'project', field: 'projectId' },
