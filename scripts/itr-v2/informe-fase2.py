...
def main():
    e = env()
    drafts = get(e, {'select': 'id,code,version,title_es,title,itr_template_sections(title),itr_template_items(item_type,requires_photo,requires_document,description_es)',
    ...
    for d in drafts:
        ...
        rows.append({'code': d['code'], 'id': d['id'], 'v': d['version'], 'active': active.get(d['code'], '—'),
                     ...
                     'title': (d.get('title_es') or d.get('title') or '')[:60]})
    ...
    with open(OUT, 'w', encoding='utf-8') as f:
        ...
        f.write(f"Ítems de inspección: {sum(r['items'] for r in rows)} · con foto: {sum(r['photos'] for r in rows)} · con equipo de prueba: {sum(1 for r in rows if r['equip'])} · sin traducción ES: {sum(r['no_es'] for r in rows)}.\n\n")
        ...
        for disc in sorted(by_disc):
            f.write(f"## {disc} — {len(by_disc[disc])} formatos\n\n| Código | Activa | Borrador | Ítems | Fotos | Equipo | Sin ES | Título |\n|---|---|---|---|---|---|---|---|\n")
            for r in by_disc[disc]:
                f.write(f"| [{r['code']}](https://commup.app/admin/templates/{r['id']}) | v{r['active']} | v{r['v']} | {r['items']} | {r['photos']} | {'sí' if r['equip'] else ''} | {r['no_es'] or ''} | {r['title']} |\n")
            f.write('\n')
