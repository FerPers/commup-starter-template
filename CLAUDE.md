...
- Offline: IndexedDB queue covers ITR responses (LWW replay) and, since Sprint O (2026-09-07), photos and punches raised from the ITR (outbox in `src/lib/offline-queue.ts`, drained by `src/lib/sync/replay.ts`; client helpers in `src/lib/sync/outbox.ts`). Signatures still require network. Sell "offline ITR execution" (installed PWA), not "full offline"
