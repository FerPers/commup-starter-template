...
- Tests: `npm test` (vitest) — unit tests next to modules (`src/**/*.test.ts`: list params, KPI aggregates, Excel header/normalize, utils, certificate eligibility) + DB tests in `tests/db/` (RLS multi-tenant smoke, `compute_system_readiness` gates, atomic punch numbering) that run only when the `TEST_SUPABASE_URL`/`TEST_USER_A|B_*`/`TEST_PROJECT_A_ID` env vars exist (GitHub secrets pending → skipped in CI for now). Certificate gate rules live in `src/lib/certificates/eligibility.ts` (pure; mirror of the SQL readiness function) — keep action, page and SQL in sync
- Offline: IndexedDB queue covers ITR responses (LWW replay) and, since Sprint O (2026-09-07), photos, punches and signatures captured in the ITR execution page (outbox in `src/lib/offline-queue.ts`, drained by `src/lib/sync/replay.ts` after responses; client helpers in `src/lib/sync/outbox.ts`; retry policy + local ITR status mirror in `src/lib/sync/outbox-utils.ts`). Rest of the app (punch list, certificates, admin) still needs network. Sell "offline ITR execution" (installed PWA), not "full offline"
