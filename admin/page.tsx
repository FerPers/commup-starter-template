...
export default function AdminDashboard() {
  ...
  const handleFileUpload = (e: any) => {
    ...
    reader.onload = (event: any) => {
      ...
      setFileData(data);
      // Simulación de conteo por disciplina
      setStats({
        total: rows.length,
        inst: rows.filter((r: any) => r[1]?.includes("Inst")).length,
        mech: rows.filter((r: any) => r[1]?.includes("Mech")).length,
        elec: rows.filter((r: any) => r[1]?.includes("Elec")).length,
      });
